...
public class HelpRamu {
    public static void main(String[] args) {
        ...
        while(t>0){
            ...
            System.out.println(findMinSumOfRupees(c1, c2, c3, c4, cabs, rickshaws));
            t--;
        }
    }
    private static int findMinSumOfRupees(int c1, int c2, int c3, int c4, int[] cabs, int[] rickshaws){
        int c_rides = 0;
        int r_rides = 0;
        for(int i=0;i<cabs.length;i++){
            c_rides+=Math.min(cabs[i]*c1, c2);
        }
        for(int i=0;i<rickshaws.length;i++){
            r_rides+=Math.min(rickshaws[i]*c1, c2);
        }
        c_rides = Math.min(c_rides, c3);
        r_rides = Math.min(r_rides, c3);
        return Math.min(c_rides + r_rides, c4);
    }
}
