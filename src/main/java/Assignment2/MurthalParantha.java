...
public class MurthalParantha {
    public static void main(String[] args) {
        ...
        int[] cook_rank = new int[cooks];
        for(int i=0;i<cooks;i++){
            cook_rank[i] = sc.nextInt();
        }
        Arrays.sort(cook_rank);
        int low = 0;
        int high = ((cook_rank[cooks-1])*(parantha*(parantha+1)))/2;
        int ans = 0;
        while (low<=high){
            int mid = (low+high)/2;
            if(isPossible(cook_rank, parantha, mid)){
                ans = mid;
                high = mid-1;
            }
            else{
    ...
    private static boolean isPossible(int[] cook_rank, int parantha, int mid){
        int t=0;
        int p=0;
        int i=0;
        int count=1;
        while(i<cook_rank.length){
            if(t+(count*cook_rank[i])<=mid){
                t+=count*cook_rank[i];
                p++;
                count++;
            }
